<!-- <div class="col-12 mx-0 px-0"><h4>Recommended Courses</h4></div> -->
    <?php foreach($all_keywords as $k){
        $keyword=$k['keyword'];
        $geththecourses=$this->db->query("select distinct * from x_cou_courses,x_cou_prov,x_cou_datav where (cou_name like '$keyword%' || cou_merchant_name like '$keyword%' || '$keyword' IN (cou_key) ) and cou_status='1' and cou_name !='' and cou_api=pr_id and cou_version=dtv_id and dtv_status='1' limit 20 ");
        $thecourse_data=$geththecourses->row_array();
        if(@$thecourse_data['cou_id'] !=''){
          $cou_img=get_cou_image($thecourse_data['cou_img'], $thecourse_data['pr_image']);
        ?>
     <div class="row shadow_3 bg-white p-2 mb-1">
        <div class="the_imase_f my-auto">
            <div style="position:absolute;left: 20px;margin-top: 10px;">
                <?=favourite_me($thecourse_data['cou_id']);?>
            </div>
        
        <a href="<?=base_url('apply/')?><?=$thecourse_data['cou_id']?>/<?=url_smart($thecourse_data['cou_name'])?>" target='_blank' >
            <img src="<?=$cou_img?>" alt="<?=$thecourse_data['cou_name']?>" class='img-fluid my-auto'>
        </a>
